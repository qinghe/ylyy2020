...
class Install extends Controller
{
	...
	public function setup3(){
		if (request()->isPost()) {
            ...
            if ($username == '') {
                echo "<script>alert('云平台账号不能为空~');history.go(-1);</script>";
            	exit();
            }
            if ($password == '') {
                echo "<script>alert('云平台密码不能为空~');history.go(-1);</script>";
            	exit();
            }
            $data = [];
            ...
            if (isset($res['code']) && $res['code'] == 1) {
                ...
                echo "<script>alert('".CONF_PATH.DS.'extra'.DS.'cloud.php写入失败！'."');history.go(-1);</script>";
            	exit();
            }else{
            	echo "<script>alert('用户名密码验证失败');history.go(-1);</script>";
            	exit();
            }
		}
	}
