    ...
    switch (config('sys.url_model')) {
        case '1'://动态
            if (config('sys.wap_levelurl')) {
                ...
                $url = config('sys.site_protocol')."://m.".config('sys.site_levelurl').str_replace('s=index/', '/', $url);
            }else{
...
function getHomeurl(){
    ...
    switch (config('sys.url_model')) {
         case '1'://动态
            if ($area) {
                $url = config('sys.wap_levelurl') ? "m.".config('sys.site_levelurl')."?area=".$area['etitle'] : config('sys.site_url').'/index.php?s=wap&area='.$area['etitle'];
            }else{
                $url = config('sys.wap_levelurl') ? "m.".config('sys.site_levelurl') : config('sys.site_url').'/index.php?s=wap';
            }
            break;
        case '3'://伪静态
...
function getTagurl($tag){
    ...
    switch (config('sys.url_model')) {
         case '1'://动态
            if ($area) {
                $url = "/index.php?s=wap/tag/index&area=".$area['etitle'];
            }else{
                $url = "/index.php?s=wap/tag/index";
            }
            $url = $url."&title=".$tag;
            if ($area['isurl']) {
